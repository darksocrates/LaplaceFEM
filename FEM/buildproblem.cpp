...
/*
 ...
 A - The assembled stiffness matrix for the FEM for poissons with linear interpolation in CSR format. Boundary conditions are enforced.
 
 RHS - The assembled RHS for the FEM with enforced boundary conditions
 */
...
std::tuple<SparseMatrix<double,RowMajor>,VectorXd> buildproblem(vector<vector<int>>& nodes, vector<vector<double>>& nodepoints, vector<int>& boundarynodes, double (*fun)(double,double), double (*bc)(double,double))
{
    ...
    //now iterate through sparse matrix and enforce direchlet boundary conditions. I choose lazy method, >_<
